...
#define BOOT_42_OFFSET 0xA00
#define BOOT_42_ARCHITECTURE_BLACKWELL 0x0000001A
...
static int nvgrace_gpu_wait_device_ready_blackwell_next(struct pci_dev *pdev)
{
...
static u32 nvgrace_gpu_arch(void __iomem *io)
{
	u32 reg = ioread32(io + BOOT_42_OFFSET);

	return FIELD_GET(GENMASK(29, 24), reg);
}

static int nvgrace_gpu_wait_device_ready(struct pci_dev *pdev,
					 void __iomem *io)
{
	u32 arch = nvgrace_gpu_arch(io);

	if (arch > BOOT_42_ARCHITECTURE_BLACKWELL)
		return nvgrace_gpu_wait_device_ready_blackwell_next(pdev);
